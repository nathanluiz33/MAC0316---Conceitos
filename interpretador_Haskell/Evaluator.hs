...
eval exp env = case exp of
  ...
  QuoteC sym -> SymV sym
  -- CaseC num intList codeList -> eval (findExp num intList codeList) env
  -- --   -- temos que acessar o elemento
  --   where
  --     findExp num intList codeList =
  --       case eval intList env of
  --         ConsV x xs -> 
  --           case eval codeList env of
  --             ConsV y ys -> 
  --               if x == num
  --                 then y
  --                 else findExp num xs ys
  --             _ -> error "As listas nao tem mesmo tamanho"
  --         _ -> error "Não achou case"
