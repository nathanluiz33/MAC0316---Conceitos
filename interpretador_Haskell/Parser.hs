...
analyze tree = case tree of
  ...
  Pair first rest -> case first of
    ...
    Leaf "case" -> CaseS (analyzePos 1) (analyzePos 2) (analyzePos 3)
    ...
    where
      ...
      check_ok_symbol :: String -> Bool
      check_ok_symbol [] = False
      check_ok_symbol (a:[]) = not (isNumeral [a])
      check_ok_symbol (a:xs)
        | isNumeral [a] = False
        | otherwise = True
